...
def _format_times(times: List[datetime.time]) -> str:
    # sufiks Z = UTC (bo scheduler pracuje na UTC)
    return ", ".join(f"{t.hour:02d}:{t.minute:02d}Z" for t in times)

def _next_occurrence_utc(now: datetime.datetime, times: List[datetime.time]) -> datetime.datetime:
    today_targets = [datetime.datetime.combine(now.date(), t) for t in times]
    future = [dt for dt in today_targets if dt > now]
    if future:
        return min(future)
